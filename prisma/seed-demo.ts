...
But the food was the star. Found a trattoria in Trastevere — cacio e pepe that was so simple and so perfect. Carbonara at Da Enzo (worth the hour wait). Gelato from Fatamorgana twice a day, no regrets.`,
  ...
  await prisma.user.update({
    ...
    data: {
      onboarded: true,
      homeAirports: JSON.stringify(["JFK", "LGA", "EWR"]),
      loyaltyPrograms: JSON.stringify(["AA", "MB"]),
      heldCards: JSON.stringify(cards.map((c) => c.id)),
      tripStyles: JSON.stringify(["culture", "food", "adventure", "nature"]),
      cabin: "economy",
      maxTravelHours: 18,
