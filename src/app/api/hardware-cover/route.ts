...
async function fetchImageBytes(url: string): Promise<Buffer> {
  ...
  if (hasPlayButtonOverlay(url)) {
    try {
      buf = Buffer.from(await declutterPlayButton(buf));
    } catch {
      /* keep original */
    }
  }
...
async function encodeCover(input: Buffer, format: string): Promise<{ body: Buffer; contentType: string }> {
  ...
  if (format === "rgb565") {
    ...
    const buf565 = Buffer.allocUnsafe(pixels.length * 2);
    for (let p = 0; p < pixels.length; p++) {
      buf565.writeUInt16LE(pixels[p], p * 2);
    }
    return { body: buf565, contentType: "application/octet-stream" };
  }
...
export async function GET(req: NextRequest) {
  ...
  for (const coverUrl of candidates) {
    ...
    try {
      ...
      return new NextResponse(new Uint8Array(body) as unknown as BodyInit, {
        status: 200,
        headers: {
